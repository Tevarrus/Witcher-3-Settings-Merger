# ==============================================================================
# Application: Witcher 3 Settings Merger
# Version:     0.9.0 (Beta)
# Author:      Craeven/Tevarrus (handles)
# License:     Copyright © 2026 Craeven/Tevarrus (handles). All Rights Reserved.
#              Distribution: No upload without permission.
#              Modification: No modification or asset use without permission.
#              Commercial Use: No commercial use allowed.
# ==============================================================================
...
# Sections that should usually be ignored during merge to prevent corruption or unnecessary locking
IGNORED_SECTIONS = [
    "[Version]", 
    "[Gameplay/EntityPool]", 
    "[Engine]",
...
# UI Styling Constants (Colors & Fonts)
# Dark Theme Palette
THEME_BG_MAIN = "#202020"
THEME_BG_CONTENT = "#2b2b2b"
THEME_TXT_HEADER = "#ffffff"
THEME_TXT_BODY = "#e0e0e0"
THEME_BTN_BG = "#333333"
THEME_BTN_HOVER = "#444444"
THEME_BTN_ACTIVE = "#1a1a1a"
THEME_ENTRY_BG = "#1f1f1f"
THEME_ENTRY_BORDER = "#3a3a3a"
THEME_FONT_MAIN = ("Segoe UI", 10)

# Pixel Asset Generation Colors
ASSET_C_BG = "#2b2b2b" # Background
ASSET_C_BD = "#666666" # Border
ASSET_C_FG = "#ffffff" # Check/Dot

# Input Constants
# List of valid input keys for The Witcher 3 to allow valid remapping suggestions.
W3_VALID_INPUTS = [
    # Keyboard - Letters
...
def resource_path(relative_path):
    ...
    TRUST NOTE: Handles asset loading for both Python interpreter and Nuitka/PyInstaller compiled builds.
    This ensures that icons and data files are correctly located whether running as source or exe.
    """
...
def get_drives():
    ...
    Used to scan for game installation across all drives.
    """
    drives = []
...
def get_true_documents_path():
    """
    Retrieves the actual path to the user's My Documents folder via Windows Shell API.
    This is more robust than using standard environment variables which can vary.
    """
    CSIDL_PERSONAL = 5
...
def apply_dark_title_bar(window):
    ...
    This improves aesthetic consistency with the dark theme GUI.
    """
    try:
        window.update()
...
def center_on_screen(window, width=None, height=None):
    """
    Centers the window on the screen based on screen dimensions.
...
def setup_popup_geometry(window, parent, mode="secondary"):
    """
    Configures popup window geometry based on mode:
    - 'secondary': Match Parent Height, Wider Width, Adjacent Position (for Conflict/Help windows).
    ...
    if match:
        ...
        # Fallback if geometry string fails
...
def position_window_adjacent(window, parent):
    ...
    # Calculate Right Position with 10px Gap
...
def find_steam_libraries():
    """
    Scans the Windows Registry to locate Steam and its library folders.
    Reads 'libraryfolders.vdf' to find additional install locations.
    """
    libraries = []
    steam_path = None
    try:
...
def find_game_path_robust(logger_func=None):
    """
    Attempts to automatically locate 'The Witcher 3' installation directory.
    Strategies:
    1. Registry (Steam Uninstall Key)
    2. Registry (GOG Key)
    3. Steam Library Folders
    4. Brute-force scan of common paths on all drives
    """
...
# ==============================================================================
# CORE LOGIC: FILE PARSING & MERGING
# ==============================================================================

def is_valid_settings_file(filepath):
    """
    Validation check to ensure the file looks like a Witcher 3 settings file.
    Checks for the presence of a [Section] header within the first 20 lines.
    """
    try:
        # SECURITY NOTE: Reading file header (Read-Only) to validate format.
...
def scan_game_for_fragments(game_path, logger_func):
    """
    Recursively scans the Game Directory to find settings files provided by mods.
    Filters out system folders (bin/x64) and core config files (r4game).
    """
    fragments = {'input': [], 'user': []}
    logger_func(f"Scanning mods in: {game_path}")
    
    # SECURITY NOTE: Performing read-only directory traversal to locate mod files.
    # We do NOT modify any files in the game directory during this scan.
    for root, dirs, files in os.walk(game_path, followlinks=True):
        for file in files:
            lower_name = file.lower()
            full_path = os.path.join(root, file)
            
            # Skip core game configs to avoid re-merging vanilla instructions
            if "config" in root.lower() and "r4game" in root.lower(): continue
            if "bin" in root.lower() and "x64" in root.lower(): continue

            # Identify Input Settings (Keybinds)
            ...
            # Identify User Settings (Graphics/Gameplay options)
...
def parse_file_to_dict(filepath, source_name="Unknown"):
    """
    Parses a settings file into a structured dictionary.
    Structure: data[section] = [ {'line': raw_line, 'key': extracted_key, 'source': source_name} ]
    """
    data = {}
    current_section = None
    try:
        # SECURITY NOTE: Reading sensitive user config files (Read-Only).
        with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
            for line in f:
                clean = line.strip()
...
def analyze_conflicts(base_data, mods_data, current_data_on_disk, review_mode=False):
    """
    Core Logic: Compares mod requirements against baseline strings.
    - If a setting is unique or matches the baseline, it's auto-resolved.
    - If multiple mods request DIFFERENT values for the same key, it's a conflict.
    - If review_mode is True, treats ANY difference as a change to review.
    """
    final_map = {}

    def normalize(s): return s.replace(" ", "").replace("\t", "")

    # Optimization: Only process keys that are actually modified by at least one Mod.
    touched_keys = set()
    ...
    def add_to_map(data_dict, check_touched=False):
        for section, entries in data_dict.items():
            for entry in entries:
                k = entry['key']
                if check_touched and (section, k) not in touched_keys: continue

                if section not in final_map: final_map[section] = {}
                if k not in final_map[section]: final_map[section][k] = []
                
                # Check uniqueness based on line content to avoid duplicates
                exists = False
    ...
    # 1. Load Baseline (Filtered by what mods actually touch)
    ...
    for section, keys in final_map.items():
        ...
        for k, entries in keys.items():
            # STATE DETECTION: Which entry is active in the file on disk?
            ...
            # Mark entries as selected if they match current disk state
            for entry in entries:
            ...
            # Handle "Custom Values" (user manually edited file, not in any mod)
            ...
            for line_norm in active_lines_normalized:
                if line_norm not in known_lines_normalized:
                    # Find original raw line
                    raw_line = ""
                    for item in current_data_on_disk[section]:
                        if item['key'] == k and normalize(item['line']) == line_norm:
                            raw_line = item['line']; break
                    
                    entries.append({
                        'line': raw_line, 'key': k,
                        'source': 'Current Custom Value', 'selected': True
                    })
            
            # Relevance Filter: Ignore keys that are purely Vanilla/Baseline with no Mod changes
            is_relevant = False
            for e in entries:
                src = e['source']
            ...
            if not is_relevant:
                ...
                is_conflict = False
                if review_mode:
                    # Show everything involved
                    if len(entries) > 1: is_conflict = True
                else:
                    # Conflict Logic: Are there >1 UNIQUE Mod sources?
                    mod_sources = set()
                    for e in entries:
                         if e['source'] not in ["Baseline/Vanilla", "Current File", "Current Custom Value"]:
                             mod_sources.add(e['source'])
                    
                    if len(mod_sources) > 1: is_conflict = True
...
def apply_merge(resolved_lines, target_full_path, app_instance):
    """
    Writes the final merged content to the target file.
    """
    output_lines = []
    ...
    # SECURITY NOTE: WRITING to user's Documents folder. 
    # This overwrites the configuration file with the merged result.
...
def merge_smart(target_full_path, fragment_files, app_instance, review_mode=False):
    ...
    if has_conflicts:
        ...
        def open_window():
            # NOTE: Depends on ConflictWindow being available in global scope (Part 4)
...
def save_baseline_file(target_full_path, logger_func):
    """
    Creates a .base backup of the specified file.
    """
    if not os.path.exists(target_full_path): return
    base_file = target_full_path + ".base"
    # SECURITY NOTE: Duplicating file for backup purposes (Write to Documents).
...
def load_baseline_file(target_full_path, logger_func):
    """
    Restores the .base backup to the live file.
    """
    base_file = target_full_path + ".base"
    if os.path.exists(base_file):
        # SECURITY NOTE: Restoring file from backup (Overwrite live file).
...
def create_pixel_assets():
    ...
    # Center Dot
    img_rad_on.put(C_FG, to=(4,4,10,10))
...
class DarkDialog(tk.Toplevel):
    ...
    def __init__(self, parent, title, message, buttons=["OK"], icon="info"):
        ...
        self.title(title)
        self.configure(bg=THEME_BG_MAIN)
        ...
        lbl_icon = tk.Label(frame, text=icon_char, font=("Segoe UI", 24), bg=THEME_BG_MAIN, fg=icon_color)
...
class RemapDialog(tk.Toplevel):
    """
    Dialog allowing user to choose a new key from the supported list.
    """
    def __init__(self, parent, current_key):
        ...
        # UI Elements
        tk.Label(self, text=f"Remapping: {current_key}", font=("Segoe UI", 10, "bold"), bg=THEME_BG_MAIN, fg=THEME_TXT_BODY).pack(pady=10)
        
        # Search Frame
        search_frame = tk.Frame(self, bg=THEME_BG_MAIN)
        search_frame.pack(fill="x", padx=10, pady=5)
        
        self.search_var = tk.StringVar()
        self.search_var.trace("w", self._filter_list)
        entry = tk.Entry(search_frame, textvariable=self.search_var, bg=THEME_BTN_BG, fg=THEME_TXT_HEADER, insertbackground="white", font=("Consolas", 10))
        entry.pack(fill="x")
        entry.focus_set()
        
        # Listbox Frame
        list_frame = tk.Frame(self, bg=THEME_BG_MAIN)
        list_frame.pack(fill="both", expand=True, padx=10, pady=5)
        
        self.listbox = tk.Listbox(list_frame, bg=THEME_BG_CONTENT, fg=THEME_TXT_BODY, font=("Consolas", 10), selectbackground="#404040", highlightthickness=0, borderwidth=0)
        ...
        # Buttons
        btn_frame = tk.Frame(self, bg=THEME_BG_MAIN)
        ...
        tk.Button(btn_frame, text="Cancel", command=self.destroy, bg=THEME_BTN_BG, fg="white", relief="flat", padx=15).pack(side="right", padx=20)
...
class ConflictWindow(tk.Toplevel):
    """
    Complex scrollable window for resolving settings conflicts.
    Supports both Checkbox logic (multiple selections) and Radio logic (single selection).
    """
    def __init__(self, parent, file_type, conflicts):
        ...
        # 2. Scrollable Area
        self.canvas = tk.Canvas(self.main_frame, bg=THEME_BG_CONTENT, highlightthickness=0)
        ...
        self.scroll_frame = tk.Frame(self.canvas, bg=THEME_BG_CONTENT)
    ...
    def _populate_list(self):
        ...
        for section, keys in self.conflicts.items():
            ...
            # --- SECTION HEADER ---
            lbl = tk.Label(self.scroll_frame, text=section, font=("Consolas", 10, "bold"), bg=THEME_BTN_BG, fg=THEME_TXT_HEADER)
            lbl.grid(row=row_idx, column=0, columnspan=5, sticky="ew", pady=(15, 5))
            row_idx += 1
            
            for key, options in keys.items():
                # --- GROUP HEADER ROW ---
                h_style = {"bg": THEME_BG_CONTENT, "fg": "#888888", "font": ("Segoe UI", 8, "bold")}
                tk.Label(self.scroll_frame, text="", **h_style).grid(row=row_idx, column=0, sticky="w")
                ...
                for idx, opt in enumerate(options):
                    ...
                    tk.Label(self.scroll_frame, text=f"[{source}]", font=("Segoe UI", 9), bg=THEME_BG_CONTENT, fg="#888888")\
                        .grid(row=row_idx, column=1, sticky="w", padx=5)

                    tk.Label(self.scroll_frame, text=action_val, font=("Segoe UI", 9, "bold"), bg=THEME_BG_CONTENT, fg="#ffffff")\
                        .grid(row=row_idx, column=2, sticky="w", padx=5)
                        
                    lbl_input = tk.Label(self.scroll_frame, text=key, font=("Segoe UI", 9), bg=THEME_BG_CONTENT, fg="#aaaaaa")
                    lbl_input.grid(row=row_idx, column=3, sticky="w", padx=5)
                        
                    if is_input_file:
                        if self.key_icon:
                            btn_remap = tk.Button(self.scroll_frame, image=self.key_icon, bg=THEME_BTN_BG, activebackground="#444444", 
                                                  relief="flat", borderwidth=0,
                                                  command=lambda s=section, k=key, i=idx, l=lbl_input: self._on_remap(s, k, i, l))
                        else:
                            btn_remap = tk.Button(self.scroll_frame, text="✏️", font=("Segoe UI", 8), bg=THEME_BTN_BG, fg="white", 
                                                  relief="flat", width=3,
                                                  command=lambda s=section, k=key, i=idx, l=lbl_input: self._on_remap(s, k, i, l))
    ...
    def _on_remap(self, section, key, idx, lbl_widget):
        ...
        if dlg.result:
             new_key = dlg.result
             lbl_widget.configure(text=new_key, fg="#ffff00") 
    ...
    def bulk_select(self, mode):
        """
        Bulk Toggle Logic for Checkboxes and Radios.
        """
        for key_tuple, var in self.vars.items():
            if len(key_tuple) == 3: # Checkbox
...
class MergerApp:
    def __init__(self, root):
        ...
        self.root.configure(bg=THEME_BG_MAIN)
        ...
        # --- DARK THEME CONFIG ---
        style.configure(".", background=THEME_BG_MAIN, foreground=THEME_TXT_BODY, font=THEME_FONT_MAIN)
        style.configure("TLabel", background=THEME_BG_MAIN, foreground=THEME_TXT_BODY)
        style.configure("TFrame", background=THEME_BG_MAIN)
        
        # Labelframe styles
        style.configure("TLabelframe", background=THEME_BG_MAIN, bordercolor=THEME_BTN_BG)
        style.configure("TLabelframe.Label", background=THEME_BG_MAIN, foreground=THEME_TXT_HEADER, font=("Segoe UI", 10, "bold"))
        ...
        style.map("TButton",
                  background=[('active', THEME_BTN_HOVER), ('pressed', THEME_BTN_ACTIVE)],
        ...
        # Custom Image-Based Checkboxes/Radios
        self.assets = create_pixel_assets() 
        
        # Define Elements
        style.element_create("DarkCheck.Indicator", "image", self.assets[0],
                             ("selected", self.assets[1]), ("active", self.assets[0]))
                             
        style.element_create("DarkRadio.Indicator", "image", self.assets[2],
                             ("selected", self.assets[3]), ("active", self.assets[2]))
        ...
        style.configure("Dark.TCheckbutton", background=THEME_BG_MAIN, foreground=THEME_TXT_BODY, focuscolor=THEME_BG_MAIN)
        style.map("Dark.TCheckbutton", background=[('active', THEME_BG_MAIN)])
        ...
        style.configure("Dark.TRadiobutton", background=THEME_BG_MAIN, foreground=THEME_TXT_BODY, focuscolor=THEME_BG_MAIN)
        style.map("Dark.TRadiobutton", background=[('active', THEME_BG_MAIN)])
        ...
        style.configure("Seamless.TCheckbutton", 
                        background=THEME_BG_CONTENT, 
                        foreground=THEME_TXT_BODY, 
                        focuscolor=THEME_BG_CONTENT,
                        indicatorbackground=THEME_BG_CONTENT,
                        indicatorforeground="#ffffff")
                        
        style.map("Seamless.TCheckbutton", 
                  background=[('active', THEME_BG_CONTENT)],
        ...
        # --- HEADER / BANNER ---
        self.banner_img = self.load_banner() 
        
        if self.banner_img:
            banner_lbl = ttk.Label(root, image=self.banner_img, background=THEME_BG_MAIN)
            banner_lbl.pack(side="top", fill="x", padx=0, pady=0)
        else:
            title_frame = ttk.Frame(root, padding="10 5 10 5")
            title_frame.pack(fill="x")
            ttk.Label(title_frame, text="Witcher 3 Settings Merger", 
                      font=("Segoe UI", 14, "bold"), foreground=THEME_TXT_HEADER).pack(side="left")
        ...
        self.entry_game = tk.Entry(path_frame, relief="flat", bg=THEME_ENTRY_BG, fg="#ffffff", insertbackground="white")
        ...
        self.entry_docs = tk.Entry(path_frame, relief="flat", bg=THEME_ENTRY_BG, fg="#ffffff", insertbackground="white")
    ...
    def log(self, msg):
        ...
        if self.log_to_file_var.get():
            try:
                # SECURITY NOTE: Logging to local file if enabled by user.
    ...
    def check_paths_live(self, event=None):
        ...
        if game_path and os.path.exists(game_path):
            self.entry_game.config(bg=THEME_ENTRY_BG, fg="#ffffff")
        else:
            self.entry_game.config(bg="#500000", fg="#ffffff") 

        docs_path = self.entry_docs.get()
        if docs_path and os.path.exists(docs_path):
            self.entry_docs.config(bg=THEME_ENTRY_BG, fg="#ffffff")
        else:
            ...
            for exe in exe_candidates:
                if os.path.exists(exe):
                    try:
                        ...
                        # SECURITY NOTE: Launching game executable.
    ...
    def load_banner(self):
        """
        Loads 'banner.png' or 'banner.jpg' from resource path or cwd.
        Resizes to 600px width.
        """
        ...
        target = None
        for c in candidates:
            # TRUST NOTE: Using resource_path to support OneDir builds.
    ...
    def show_help(self):
        ...
        help_win.configure(bg=THEME_BG_MAIN)
